    ...
    , ui(new Ui::MainWindow)
{
    ...
    currentSubwindow = ui->mdiArea->addSubWindow(new MyTextEdit(this));
...
void MainWindow::on_saveButton_clicked()
{
    ...
    if (!fileName.isEmpty())
    {
        ...
        if (file.open(QIODevice::WriteOnly))
        {
            ...
            stream << dynamic_cast<MyTextEdit*>(currentSubwindow->widget())->toPlainText();
...
void MainWindow::on_openButton_clicked()
{
    ...
    if (!fileName.isEmpty())
    {
        ...
        {
            QTextStream stream(&file);
            dynamic_cast<MyTextEdit*>(currentSubwindow->widget())->setPlainText(stream.readAll());
//            ui->plainTextEdit->setPlainText(stream.readAll());
            file.close();
            dynamic_cast<MyTextEdit*>(currentSubwindow->widget())->setReadOnly(false);
...
//            ui->plainTextEdit->setPlainText(stream.readAll());
            file.close();
            dynamic_cast<MyTextEdit*>(currentSubwindow->widget())->setReadOnly(true);
...
//    ui->mdiArea->setActiveSubWindow(currentSubwindow);
    dynamic_cast<MyTextEdit*>(currentSubwindow->widget())->clear();
...
void MainWindow::on_printButton_clicked()
{
    ...
    if (printDialog.exec() != QDialog::Accepted)
    	return;
    dynamic_cast<MyTextEdit*>(currentSubwindow->widget())->print(&printer);
...
MyTextEdit::MyTextEdit(QWidget *parent) : QTextEdit(parent)
{
    ...
    connect(contextMenu->addAction(tr("Копировать")), &QAction::triggered, this, &MyTextEdit::copy);
    connect(contextMenu->addAction(tr("Вставить")), &QAction::triggered, this, &MyTextEdit::paste);

    contextMenu->addSeparator();

    QMenu* alignementMenu = contextMenu->addMenu(tr("Выровнять по..."));
    connect(alignementMenu->addAction(tr("Левому краю")), &QAction::triggered, this, &MyTextEdit::alignLeft);
    connect(alignementMenu->addAction(tr("Правому краю")), &QAction::triggered, this, &MyTextEdit::alignRight);
    connect(alignementMenu->addAction(tr("Центру")), &QAction::triggered, this, &MyTextEdit::alignCenter);
...
void MyTextEdit::alignRight()
{
    setAlignment(Qt::AlignRight);
}

void MyTextEdit::alignCenter()
{
